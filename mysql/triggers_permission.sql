...
CREATE TRIGGER `dbee`.`insert_permission_audit`
  AFTER INSERT
  ON `dbee`.`permission`
  FOR EACH ROW
  BEGIN
    ...
    INSERT INTO `dbee`.`permission_audit` (`action`,
                                      `audit_id`,
                                      `audit_created_by`,
                                      `audit_created_date`,
                                      `audit_updated_by`,
...
CREATE TRIGGER `dbee`.`update_permission_audit`
  AFTER UPDATE
  ON `dbee`.`permission`
  FOR EACH ROW
  BEGIN
...
CREATE TRIGGER `dbee`.`delete_permission_audit`
  AFTER DELETE
  ON `dbee`.`permission`
  FOR EACH ROW
  BEGIN
